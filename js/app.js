const app = {
    ...
    navigate: (viewId) => {
        ...
        if (viewId === 'add') document.getElementById('item-codigo').value = utils.generateCode();
        if (viewId === 'scanner') app.startScanner();
    },

    toggleLogin: () => {
        if (app.isLoggedIn) {
            app.isLoggedIn = false; app.currentUser = null;
            localStorage.removeItem('sessionUser');
    ...
    saveItem: async (e) => {
        ...
        const item = {
            ...
            foto: fotoBase64, // Salva localmente primeiro (Base64)
        ...
        await db.save(item);
        alert('Item salvo! (A foto irá para a nuvem na próxima sincronização)');
    ...
    renderDetail: async (codigo) => {
        ...
        container.innerHTML = `
            ...
            <div class="mt-4"><h4 class="font-bold text-sm mb-2">Histórico</h4><ul class="space-y-1">${historicoHtml}</ul></div>
        `;

        setTimeout(() => {
            new QRCode(document.getElementById("detail-qrcode"), { text: item.codigo, width: 150, height: 150 });
        }, 100);

        const btnEditObs = document.getElementById('btn-edit-obs');
        if (app.isLoggedIn && app.currentUser && app.currentUser.level === 'admin') btnEditObs.classList.remove('hidden');
        else btnEditObs.classList.add('hidden');

        if (app.isLoggedIn) document.getElementById('admin-actions').classList.remove('hidden');
        else document.getElementById('admin-actions').classList.add('hidden');
        
        app.navigate('detail');
    },

    editObservation: async () => {
        if (!app.isLoggedIn || !app.currentUser || app.currentUser.level !== 'admin') { alert('Apenas administradores.'); return; }
    ...
    updateStatus: async (newStatus) => {
        ...
        } else if (newStatus === 'Manutenção') { obs = prompt('Motivo / Nº OS:') || '-'; }
    ...
    printLabels: async () => {
        const items = await db.getAll();
        if (items.length === 0) { alert('Nenhum item cadastrado.'); return; }
        const printWindow = window.open('', '_blank');
        let labelsHtml = '';
        items.forEach(item => {
            const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(item.codigo)}`;
            labelsHtml += `<div class="label"><img src="${qrUrl}" alt="QR Code" class="qr-img"><div class="label-text"><div class="label-code">${item.codigo}</div><div class="label-desc">${item.descricao}</div></div></div>`;
        });
        printWindow.document.write(`
            <html><head><title>Etiquetas</title><style>
                ...
                <div class="no-print" style="text-align:center; margin-bottom:20px;">
                    <h2>Etiquetas (${items.length} itens)</h2>
        ...
        app.scanner.start({ facingMode: "environment" }, { fps: 10, qrbox: { width: 250, height: 250 } },
            async (decodedText) => {
                ...
                if (!app.isLoggedIn) { alert(`Código: ${decodedText}\n(Faça login para ver detalhes)`); app.navigate('dashboard'); } 
                    ...
                    if (item) app.renderDetail(decodedText); else { alert('Item não encontrado.'); app.navigate('dashboard'); }
                }
    ...
    openUserManagement: () => {
        ...
        users.forEach(user => {
            ...
            div.innerHTML = `<div><p class="font-bold">${user.name}</p><p class="text-xs text-gray-600">@${user.username} - ${app.accessLevels[user.level].name}</p></div>${user.username !== 'admin' ? `<button onclick="app.deleteUser('${user.username}')" class="text-red-600 text-sm">Excluir</button>` : ''}`;
            container.appendChild(div);
        });
    ...
    createUser: () => {
        ...
        alert('Usuário criado!'); app.openUserManagement();
    },

    deleteUser: (username) => {
        if (confirm(`Excluir ${username}?`)) { app.users.delete(username); app.openUserManagement(); }
    },
