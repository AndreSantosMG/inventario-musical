const app = {
    ...
    localUsers: [], // Cache de usuários da nuvem
    ...
    init: async () => {
        ...
        // Carrega usuários da nuvem (ou cache) em background
    ...
    forceUpdate: () => {
        if (confirm('Isso vai limpar o cache e recarregar.\n\nUSUÁRIOS e UNIDADES serão PRESERVADOS.\n\nOK?')) {
            ...
            for (let i = 0; i < localStorage.length; i++) {
                ...
                if (key && (key.startsWith('user_') || key.startsWith('inst_') || key === 'cloudUsersCache' || key === 'cloudUsersLastSync')) {
    ...
    openLoginModal: () => {
        try {
            app.users.init(); app.instituicoes.init();
    ...
    doLogin: async () => {
        ...
        if (!username) { alert('Selecione ou digite seu usuário'); return; }
        ...
        // Verifica usuários da nuvem (cache local)
        const cloudUser = app.localUsers.find(u => u.username === username);
        if (!cloudUser) { alert('Usuário não encontrado. Sincronize ou contate o administrador.'); return; }
    ...
    clearData: async () => {
        ...
        if (confirm('APAGAR TUDO? USUÁRIOS E UNIDADES SERÃO PRESERVADOS.')) {
            ...
            for (let i = 0; i < localStorage.length; i++) {
                ...
                if (k && (k.startsWith('user_') || k.startsWith('inst_') || k.includes('cloudUsers'))) protect.push({ k, v: localStorage.getItem(k) });
            }
            localStorage.clear();
    ...
    openUserManagement: () => {
        if (!app.isLoggedIn || (app.currentUser.nivel || app.currentUser.level) !== 'admin') { alert('Apenas admins'); return; }
        app.users.init();
        const users = app.localUsers.filter(u => !u.master);
    ...
    createUser: async () => {
        ...
        if (res.status === 'success') {
            ...
            alert('Erro ao sincronizar usuário: ' + res.message);
        }
    },
    ...
    deleteUser: async (username) => {
        ...
        if (confirm(`Excluir ${username}?\nAcesso será revogado.`)) {
            ...
            if (idx > -1) {
                ...
                localStorage.setItem('cloudUsersCache', JSON.stringify(app.localUsers));
                // Marca como inativo na planilha
                const res = await sync.syncUsers([{ username, ativo: false }]);
                if (res.status === 'success') alert('Excluído e revogado.');
                else alert('Erro ao revogar: ' + res.message);
            }
            app.openUserManagement();
        }
    },
    ...
    users: {
        init: async () => {
            ...
            // MIGRAÇÃO: Converte usuários antigos (texto puro) para hash e envia para nuvem
            const oldKeys = [];
            for (let i = 0; i < localStorage.length; i++) {
                const key = localStorage.key(i);
            ...
            if (oldKeys.length > 0 && !localStorage.getItem('usersMigrated')) {
                ...
                if (usersToMigrate.length > 0) {
                    ...
                    app.localUsers = [...app.localUsers.filter(u => u.master), ...usersToMigrate];
                    localStorage.setItem('cloudUsersCache', JSON.stringify(app.localUsers));
                    ...
                    console.log(`✅ Migrados ${usersToMigrate.length} usuários para a nuvem.`);
                }
            }
        },
        getLocal: (username) => {
